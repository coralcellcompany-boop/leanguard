...
export function trends(weights: Weight[], sets: Performance[], now = new Date()) {
  ...
  const recentWeights = weights.filter((x) => Date.parse(x.recorded_at) >= boundary && Date.parse(x.recorded_at) <= now.getTime());
  const priorWeights = weights.filter((x) => Date.parse(x.recorded_at) >= start && Date.parse(x.recorded_at) < boundary);
