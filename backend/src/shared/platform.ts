...
import { db, type DocumentData, type Transaction } from "../database/postgres.js";
export { db, withUserDb, withUserSql, checkDatabase, closeDatabase } from "../database/postgres.js";
export type { DocumentData, DocumentReference, Transaction } from "../database/postgres.js";
import { config, environment } from "./config.js";
import type { Request, Response } from "express";
import { HttpError } from "./errors.js";
export { HttpError } from "./errors.js";
const defineSecret = (name: string) => ({ value: () => environment(name) });
const defineString = (name: string, options: {default: string}) => ({ value: () => process.env[name] ?? options.default });
const defineBoolean = (name: string, options: {default: boolean}) => ({ value: () => process.env[name] === undefined ? options.default : process.env[name] === "true" });
...
export async function identify(req: Request, allowDeleting = false): Promise<Identity> {
  ...
  if (decoded.email_verified !== true) throw new HttpError(403, "email_verification_required", "Verify your email address before using LeanGuard.");
...
export const requireIdentity = identify;
