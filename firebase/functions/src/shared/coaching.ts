...
export function validateReply(value: unknown, snapshot: Snapshot, kind: Kind): CoachReply {
  ...
  if (/\b(ozempic|wegovy|mounjaro|zepbound|semaglutide|tirzepatide|dose|dosage|prescribe|titrate)\b/i.test(generatedText)) {
    return safetyReply("medication_boundary");
  }
  if (/\b(diagnos(?:is|e)|you (?:have|are suffering from|are experiencing) (?:sarcopenia|hypoglyc(?:emia|aemia)|dehydration|diabetes|malnutrition|an? injury)|(?:exercise|train|work out|keep going).{0,30}despite.{0,20}(?:pain|faint|dehydrat|weakness))\b/i.test(generatedText)) return safetyReply("medical_referral");
