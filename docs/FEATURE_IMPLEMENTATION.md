...
| Body measurements | Weight plus waist entry | Chest, hips, arm and thigh entry; history and comparison presentation | Measurements screen, `addMeasurement`; Node API checks new premium measurement writes |
| Weekly review | Current week numbers and one AI summary each UTC calendar week | Detailed structured review with logged evidence and up to three next actions | Reports screen, `weekly_insights`, `coach` kind `weekly`, `reserve()` PostgreSQL transaction |
...
| Reminders | Up to 3 enabled fixed local reminders | Smart push suppression after a goal is met, quiet hours and more reminders | `NotificationService`, `PushService`, `toggleReminder`, `setQuietHours`; `dispatchReminders`, private Node worker, owner-protected device tokens |
...
| Delete account | Typed DELETE, recent sign-in, server private-export removal, RevenueCat customer deletion if configured, PostgreSQL owner cleanup and Auth deletion; store billing managed separately |
...
| Privacy | Separate health/AI/analytics/crash/notification consent, private VPS export storage, per-user PostgreSQL RLS and account-scoped local snapshots |
...
`test/core/repository_test.dart` exercises account isolation, in-flight edits, offline retry, natural-key conflicts and immutable consent. `controller_test.dart` exercises onboarding, logging/skipping/completion, validation, Free gating, safety escalation, denial, restoration and AI failure. `entities_test.dart` round-trips all 20 domain models. Service and widget suites cover native boundaries and screens. The active Node backend is type-checked and tested against real PostgreSQL migrations/RLS/transactions and isolated Firebase Auth tokens, with mocked Responses transport tests. Retained Supabase and Firebase Functions code is migration reference only. See [BACKEND_VPS.md](BACKEND_VPS.md) and [VALIDATION.md](VALIDATION.md).
